  ...
  try {
    ...
    if (user.userType === 'CONTRACTOR' && user.contractorProfile) {
      const contractorEvaluations = user.receivedEvaluations.filter(
        eval => eval.evaluationType === 'CLIENT_TO_CONTRACTOR'
      )

      if (contractorEvaluations.length > 0) {
        const avgTechnical = contractorEvaluations.reduce((sum, eval) => 
          sum + (eval.technicalRating || 0), 0) / contractorEvaluations.length
        
        const avgDelivery = contractorEvaluations.reduce((sum, eval) => 
          sum + (eval.deliveryRating || 0), 0) / contractorEvaluations.length
        
        const avgCommunication = contractorEvaluations.reduce((sum, eval) => 
          sum + (eval.communicationRating || 0), 0) / contractorEvaluations.length
        
        const avgQuality = contractorEvaluations.reduce((sum, eval) => 
          sum + (eval.qualityRating || 0), 0) / contractorEvaluations.length
    ...
    if (user.userType === 'CLIENT' && user.clientProfile) {
      const clientEvaluations = user.receivedEvaluations.filter(
        eval => eval.evaluationType === 'CONTRACTOR_TO_CLIENT'
      )

      if (clientEvaluations.length > 0) {
        const avgPayment = clientEvaluations.reduce((sum, eval) => 
          sum + (eval.paymentRating || 0), 0) / clientEvaluations.length
        
        const avgEfficiency = clientEvaluations.reduce((sum, eval) => 
          sum + (eval.deliveryRating || 0), 0) / clientEvaluations.length
        
        const avgQuality = clientEvaluations.reduce((sum, eval) => 
          sum + (eval.qualityRating || 0), 0) / clientEvaluations.length
